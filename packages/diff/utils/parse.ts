...
const buildHunkHeader = (
  ...
  return `@@ -${oldRange} +${newRange} @@${trailingContext ? ` ${trailingContext}` : ""}`;
};

const lineHasOldNumber = (line: Line): boolean => line.type !== "insert";

const lineHasNewNumber = (line: Line): boolean => line.type !== "delete";

const isChangedLine = (line: Line): boolean => {
  if (line.type === "insert" || line.type === "delete") return true;

  return line.content.some(
    (segment) => segment.type === "insert" || segment.type === "delete",
  );
};

const sliceHunk = (hunk: Hunk, start: number, end: number): Hunk => {
  const lines = hunk.lines.slice(start, end);

  const firstOldLine = lines.find((line) => lineHasOldNumber(line));
  const firstNewLine = lines.find((line) => lineHasNewNumber(line));
  const oldStart =
    firstOldLine?.type === "normal"
      ? (firstOldLine.oldLineNumber ?? hunk.oldStart)
      : (firstOldLine?.lineNumber ?? hunk.oldStart);
  const newStart =
    firstNewLine?.type === "normal"
      ? (firstNewLine.newLineNumber ?? hunk.newStart)
      : (firstNewLine?.lineNumber ?? hunk.newStart);

  const oldLines = lines.reduce(
    (count, line) => count + (lineHasOldNumber(line) ? 1 : 0),
    0,
  );
  const newLines = lines.reduce(
    (count, line) => count + (lineHasNewNumber(line) ? 1 : 0),
    0,
  );
  ...
  return {
    ...
    newStart,
    newLines,
    content: buildHunkHeader(
      oldStart,
      oldLines,
      newStart,
      newLines,
      extractHunkContext(hunk.content),
    ),
    lines,
  };
};

const collapseHunkWithContext = (
  hunk: Hunk,
  contextLines: number,
): (Hunk | SkipBlock)[] => {
  ...
  if (lines.length === 0) return [hunk];

  const windows: Array<{ start: number; end: number }> = [];

  for (let i = 0; i < lines.length; i++) {
    if (!isChangedLine(lines[i])) continue;

    windows.push({
      start: Math.max(0, i - clampedContext),
      end: Math.min(lines.length - 1, i + clampedContext),
    });
  }

  if (windows.length === 0) return [hunk];
  ...
  for (const window of mergedWindows) {
    if (window.start > cursor) {
      const hiddenLines = lines.slice(cursor, window.start);
      result.push({
        count: hiddenLines.length,
        type: "skip",
        content: `${hiddenLines.length} lines hidden`,
        hiddenLines,
      });
    }

    result.push(sliceHunk(hunk, window.start, window.end + 1));
    cursor = window.end + 1;
  }

  if (cursor < lines.length) {
    const hiddenLines = lines.slice(cursor);
    result.push({
      count: hiddenLines.length,
      type: "skip",
      content: `${hiddenLines.length} lines hidden`,
...
export const parseDiff = (
  ...
  return files.map((file) => ({
    ...file,
    hunks: insertSkipBlocks(
      file.hunks.map((hunk) => parseHunk(hunk, opts)),
    ).flatMap((hunk) =>
      hunk.type === "hunk"
        ? collapseHunkWithContext(hunk, opts.contextLines)
        : hunk,
    ),
  }));
};
