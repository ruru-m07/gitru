...
#[test]
#[serial]
fn text_diff_worktree_modified_returns_old_and_new_blame() {
    run_async(async {
        let repo = TestRepo::new();
        ...
        let diff = service
            ...
            .await
            .expect("failed to get text diff");

        assert!(diff.oldBlame.is_none());
        assert!(diff.newBlame.is_none());
    });
}

#[test]
#[serial]
fn text_diff_deleted_file_returns_empty_new_blame() {
    run_async(async {
        let repo = TestRepo::new();
...
#[test]
#[serial]
fn text_diff_commit_mode_returns_old_and_new_blame() {
    run_async(async {
        let repo = TestRepo::new();
