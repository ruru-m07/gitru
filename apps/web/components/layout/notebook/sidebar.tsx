    ...
    <Base.SidebarContent>
      {({ collapsed, hovered, ref: asideRef, ...rest }) => (
        <div
          ...
          className={cn(
            'sticky z-20 [grid-area:sidebar] pointer-events-none *:pointer-events-auto md:layout:[--fd-sidebar-width:268px] max-md:hidden border-r',
            navMode === 'auto'
...
export function SidebarSeparator({ className, style, children, ...props }: ComponentProps<'p'>) {
  ...
  return (
    ...
    className={cn(
        'inline-flex items-center gap-2 mb-1.5 px-2  _text-muted-foreground text-xs mt-6 empty:mb-0 [&_svg]:size-4 [&_svg]:shrink-0',
        depth === 0 && 'first:mt-0',
        className,
      )}
      style={{
...
export function SidebarItem({
  ...
  return (
    <Base.SidebarItem
      className={cn(itemVariants({ variant: 'link', highlight: depth >= 1 }), "font-normal ml-1", className)}
      style={{
        paddingInlineStart: getItemOffset(depth),
        ...style,
      }}
